...
The `R` package `qcapower` allows you to estimate power of a term in a Qualitative Comparative Analysis (QCA), or, more specifically, a truth table analysis. This vignette introduces the package, its in-built functions and things you might want to do with the package output in addition to what the functions offers. At present, the package includes four functions:  
...
* `power`: the power estimate. The entry is the same in each row because it is the power estimate for the entire dataset.
...
The power estimate should be reliable in the sense that it should only change marginally if you add 10 or 50 additional simulations. The cumulative power plot is a visual means for checking whether `sims` is sufficiently large. This can be easily displayed with  `qp_run_plot()`. You can add a title to the figure by specifying `title=T`.

```{r qp_run_plot, fig.cap="Fig. 1: Plot of running power estimate"}
qp_run_plot(qcapower_vign)
```

In this example, we see that the estimate is oscillating aroung 0.5 after about 300 simulations and stabilizes in this range. Setting `sims` to 1000 therefore seems appropiate. If you prefer a different visualization style, you can of course create your own plot by working with the `powercum` column of your dataset.
...
The function `qp_cases()` allows you to estimate how many cases you need to achieve a target level of power. It is based on caluclated data with 5000 simulations and 50000 permutations.
...
The function `qp_cases_brute()` allows you to estimate how many cases you need to achieve a target level of power. By specifying the `power_target` and all parameters except `cases`, the function iteratively searches for the number of cases yielding a power estimate that is sufficiently close to the target level. (see above).

__Note:__ Running the function may take some time with a high number of simulations and permutations.
